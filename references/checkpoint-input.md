...
The command refreshes once, verifies checkpoint ownership/source state and repository state, and validates the embedded native plan or optional legacy `phases` plus `safety_reserve_percent` before any side effect. Only after every gate passes does it restore the native plan, clear confirmed heartbeat metadata, mark a verified automation `EXECUTED`, transition the checkpoint to working, and send `TASK_RESUMED` from the same snapshot. It derives `resume_point` from the checkpoint. If validation, repository verification, or heartbeat cleanup fails, it does not restore the plan or transition to `WORKING`; cleanup is not attempted before prevalidation. A second wake returns `ALREADY_RESUMED` without mutation or duplicate Discord.
