...
node scripts/task-guard.mjs phase prepare --provider C:\path\to\provider.mjs --project C:\path\to\project --input semantic-request.json
...
An explicitly named local semantic provider can be attached directly to the default start boundary with `phase prepare --provider MODULE`. Task Guard calls the provider, validates its structured output, persists the resulting native plan and provider provenance, refreshes quota once, and selects or starts only a graph-ready phase in the same command. `phase generate` remains a preview/diagnostic command that prints the same validated plan for inspection. The module's default export has an `id` and async `generate(request)` function. The provider supplies the semantic choices: phase IDs, phase types, dependency edges, and optional file estimates. Additional fields and malformed or invalid graphs are rejected. Provider provenance is restored with the native plan through quota checkpoints. Provider modules run as trusted local code with the same permissions as Task Guard.

Task Guard does not ship a built-in semantic model, infer phases with keyword rules, or recursively call the currently running Codex task. `--provider` cannot be combined with caller-supplied `phases` or `generation`; without a provider, explicit native plans remain the default and behave exactly as before. See [references/phase-input.md](references/phase-input.md) for the provider contract and generation examples.
