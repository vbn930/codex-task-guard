...
Before implementation, decompose the task into ordered, dependency-aware phases. Read [references/phase-input.md](references/phase-input.md). Prefer an explicit native plan with `depends_on` and optional `estimated_files`; Task Guard then validates and persists the graph and computes readiness itself. When an explicit local semantic provider is available, use `phase prepare --provider <module>` to generate, validate, persist, and quota-select the native plan in one start boundary. `phase generate` remains a preview/diagnostic command. The provider supplies the semantic decisions; Task Guard only validates and normalizes them. It persists provider provenance with the plan and quota checkpoint, but does not ship keyword heuristics or recursively invoke the current Codex task. Keep phases independently finishable and small enough to checkpoint between them. Do not redefine the task merely to fit the current quota window.
...
Use `phase prepare --project <project> --input <phase-plan.json>` with an explicit safety reserve and measured history. For a native plan, pass top-level `task_id` plus the full phase graph on first prepare; later prepare/finish calls use its persisted completion state. It refreshes once, evaluates only graph-ready phases, and records the selected phase start from that same snapshot. Run only `decision.selected_phase_id`. If no phase fits, it returns `phase_start: null` and creates no active phase. `budget evaluate` and `phase start` remain available for diagnostic/manual use, but do not compose them as the default automated workflow because separate commands do not form one lifecycle boundary. If no measured cohort exists, the result is `INSUFFICIENT_HISTORY`; do not invent a model multiplier or claim the phase is safe. Split the phase further, run only a deliberately bounded calibration phase when justified, or pause.
...
The estimator matches exact plan/model/reasoning/phase-type cohorts. When `estimated_files` is present, it records that value and estimates from the nearest measured file count that is not smaller; it never extrapolates upward from a smaller phase. Fewer than 20 valid samples use the highest observed delta. At 20 samples, it uses nearest-rank P90 from the latest 50 valid samples plus one percentage point. The ledger compacts after 2,500 records to at most 2,000, prioritizing the latest 50 records per exact cohort. It does not extrapolate across models or plans or apply official message ranges as task-cost formulas. After every completed phase, re-evaluate the pending phases against the newly observed quota.
