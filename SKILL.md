...
Before implementation, decompose the task into ordered, dependency-aware phases. Read [references/phase-input.md](references/phase-input.md). Prefer an explicit native plan with `depends_on` and optional `estimated_files`; Task Guard then validates and persists the graph and computes readiness itself. When an explicit local semantic provider is available, `phase generate` can turn task prose into that same native plan contract. The provider supplies the semantic decisions; Task Guard only validates and normalizes them. It does not ship keyword heuristics or recursively invoke the current Codex task. Keep phases independently finishable and small enough to checkpoint between them. Do not redefine the task merely to fit the current quota window.
